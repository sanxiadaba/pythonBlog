...
def home():
    ...
    result = instanceArticle.find_limit_with_users(0, 10)
    total = math.ceil((instanceArticle.get_total_count() / 10))
...
def classify(type, page):
    start = (page - 1) * 10
    result = instanceArticle.find_by_type(type, start, 10)
    total = math.ceil(instanceArticle.get_count_by_type(type) / 10)
